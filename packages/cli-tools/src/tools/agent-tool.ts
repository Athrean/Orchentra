...
import { BUILTIN_ROLES, resolveSubagentRole, restrictRegistry, type SubagentRole } from './subagent-roles'
...
/** Config-driven overrides for the nesting/fan-out caps; undefined keeps the defaults. */
...
/**
 * Builds the `agent` tool over a specific role set, so the `agentType` schema
 * enum reflects the merged built-in + discovered roles (no hand-maintained
 * literal — which is why the browser-tester role is now reachable) and the
 * depth/fan-out caps come from config. `agentTool` below is this applied to the
 * built-in roles with default caps.
 */
export function createAgentTool(roles: Record<string, SubagentRole>, caps: SubagentCaps = {}): ToolDefinition {
  const maxDepth = caps.maxDepth ?? DEFAULT_MAX_SUBAGENT_DEPTH
  const maxConcurrent = caps.maxConcurrent ?? DEFAULT_MAX_CONCURRENT_SUBAGENTS
  const agentTypes = Object.keys(roles)
  return {
    name: 'agent',
    description: `Spawn sub-agent(s) to perform a task. Each sub-agent runs a nested conversation loop with the same tools and spine, and its spend counts against the parent budget. Pass "tasks" (an array of independent task prompts) to fan out concurrent sub-agents instead of running one at a time, capped at ${maxConcurrent} running at once. Beyond ${SPAWN_JUSTIFICATION_THRESHOLD} tasks, pass "justification" explaining why parallel fan-out is warranted. When parallel tasks write code, pass isolation "worktree" so each slice runs in its own git worktree and only gated, non-overlapping slices merge back.`,
    level: 'admin',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        agentType: {
          ...
          enum: agentTypes,
          description:
            'Optional specialist role for the sub-agent(s). Built-ins: "explorer" (read-only search), "reviewer" (read + run checks, no edits), "builder" (full toolset), "browser-tester" (browser ops + bash, judges screenshots). Project- or user-defined types from .orchentra/agents are also selectable by name. Omit for a generic sub-agent. Applies to every task in a "tasks" batch.',
        },
        isolation: {
          type: 'string',
          enum: ['worktree'],
    ...
    async execute(args: unknown, ctx: ToolContext): Promise<ToolResult> {
      ...
      const { role, error: roleError } = resolveSubagentRole(input.agentType, roles)
      if (!role) {
      ...
      if (depth >= maxDepth) {
        return {
          content: `error: sub-agent recursion depth cap reached (${maxDepth}); refusing to spawn deeper`,
          isError: true,
        }
      }
      ...
      if (input.isolation === 'worktree') {
        return runWorktreeBatch(tasks, model, childCtx, role, maxConcurrent)
      }
      if (input.isolation !== undefined) {
...
/** The `agent` tool over the built-in roles with default caps (the process-wide default). */
export const agentTool: ToolDefinition = createAgentTool(BUILTIN_ROLES)
