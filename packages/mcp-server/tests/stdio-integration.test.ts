...
describe('orchentra mcp serve (stdio)', () => {
  ...
  test('tools/list returns the migrated read-scoped GitHub tools', async () => {
    ...
    // After #291 alone, this slice contributes 5 tools. Once #290 lands the
    // foundation slice (which adds get_workflow_logs), this list grows to 6.
    expect(names).toEqual(['get_commit_changes', 'get_file_content', 'get_issue', 'get_pull_request', 'search_code'])
  })
