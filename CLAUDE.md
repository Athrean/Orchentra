...
The DevOps operations backend (`apps/server`, the operations / MCP-server / execution-graph packages, Postgres) was **removed** in the CLI-only cut (branch `cleanup/cli-only`). What remains is the coding agent plus a thin **Studio** web surface (auth + usage + API keys + billing) for selling subscriptions later. The operations-contract idea — typed, trust-classed, audited units of work — stays a design reference for any future privileged action, but no longer ships.
...
| MCP **client** (consume external servers)                           | shipped             | `cli-tools/mcp` (stdio + http) wires external MCP tools into the agent loop           |
| Studio web (sell subscriptions)                                     | planned             | thin marketing + auth + usage + API keys + billing surface                            |

Every new feature must answer: **does it save tokens (input or output) or make the agent write less / better code?** If not, we don't build it. Terseness and compression are always cosmetic/transport concerns — they never alter a safety-relevant message.

---

## 2. Roadmap status

**Active roadmap lives in `docs/ORCHENTRA_PLAN.md`** (Phase S strip → efficiency layer). The DevOps foundation in the table below was **removed** in the CLI-only cut (branch `cleanup/cli-only`) — those packages (`apps/server`, `operations`, `mcp-server`, `db`, `core`) no longer exist; the table is historical provenance only.
...
| Web Surface (Studio) | `apps/web/app/`                                          | thin standalone Studio: marketing, auth, usage, API keys, billing (sell subscriptions). No import dependency on the CLI. (DevOps onboarding/repo-insights being removed.)                                                                         |
| Agent Runtime (CLI)  | `packages/cli-core/src/runtime`                          | `ConversationRuntime` — the CLI agent loop (stateless, step lifecycle, budget, spans) + provider abstraction, compaction, hooks, permissions, skills, session replay, `worker-boot`. **The** agent loop. **Where context compression grafts in.** |
