...
export class PlanModeCommand implements CommandHandler {
  ...
  async execute(args: string[], ctx: CommandContext): Promise<boolean> {
    ...
    if (action !== 'on' && action !== 'true') {
      emit(ctx, `Unknown planmode action "${action}". Use on, off, or status.`, 'warn')
      return true
    }

    ctx.session.setPlanMode(true)
    emit(ctx, 'Plan mode enabled. Tools are blocked until /planmode off.')
    return true
  }
}
